...
Responsibilities:
 ...
 - Extract relevant columns (Transaction Description, Automated Trans. Category, Transaction Type, Classification, Type)
...
def load_and_prepare_details(xlsx_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    required_cols = ["Transaction Description", "Automated Trans. Category", "Transaction Type", "Classification", "Type"]
    ...
    labeled_df = df[df["Classification"].notna() & df["Type"].notna()].copy()
    unlabeled_df = df[df["Classification"].isna() | df["Type"].isna()].copy()
